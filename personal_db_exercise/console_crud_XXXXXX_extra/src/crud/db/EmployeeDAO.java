...
public class EmployeeDAO {
	...
	/** 社員名検索用のクエリ */
	public static final String SQL_GET_BY_EMP_NAME = String.join("\n",
			"SELECT",
			"	emp_id,",
			"	emp_name,",
			"	gender,",
	...
	/** 部署ID検索用のクエリ */
	public static final String SQL_GET_BY_DEPT_ID = String.join("\n",
			"SELECT",
			"	emp_id,",
			"	emp_name,",
			"	gender,",
	...
	/** 社員データ取得用のクエリ */
	public static final String SQL_FIND_BY_EMP_ID = String.join("\n",
			"SELECT",
			"	emp_id,",
			"	emp_name,",
			"	gender,",
	...
	/**
	 * 社員IDの一致する社員データを取得します。
	 * 
	 * @param empId 検索対象の社員ID
	 * @return 一致する社員データ
	 */
	public static Employee findByEmpId(int empId) {
		try (Connection conn = ConnectionProvider.getConnection();
				PreparedStatement stmt = conn.prepareStatement(SQL_FIND_BY_EMP_ID)) {
			stmt.setInt(1, empId);
			try (ResultSet rs = stmt.executeQuery()) {
				if (rs.next()) {
					Department dept = new Department(
	...
	/**
	 * 社員名が部分一致する社員データを取得します。
	 * 
	 * @param empName 検索対象の社員名
	 * @return 社員データを含むリスト
	 */
	public static List<Employee> getByEmpName(String empName) {
		try (Connection conn = ConnectionProvider.getConnection();
				PreparedStatement stmt = conn.prepareStatement(SQL_GET_BY_EMP_NAME)) {
	...
	/**
	 * 部署IDが一致する社員データを取得します。
	 * 
	 * @param deptId 検索対象の部署ID
	 * @return 社員データを含むリスト
	 */
	public static List<Employee> getByDeptId(int deptId) {
		try (Connection conn = ConnectionProvider.getConnection();
				PreparedStatement stmt = conn.prepareStatement(SQL_GET_BY_DEPT_ID)) {
